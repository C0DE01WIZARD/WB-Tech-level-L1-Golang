...
// Создаём функцию reverseString для переворачивания строки. Возвращает строку.
	func reverseString(s string) string { 
		// Преобразуем строку в срез Unicode-символов.
			...
			i < j; i, j = i+1, j-1 { // Проверяем условие i < j. Пока i меньше j, цикл будет выполняться.
			...
			//После каждого цикла индексы i и j увеличиваются на единицу и уменьшаются на единицу. Это позволяет нам последовательно пройтись по всем элементам среза и перевернуть строку.
			return string(runes)
	}
	...
	func main() { 
			...
			//Вариант №1 
			var String1 string = "главрыба" // Определяем переменную String со значение "главрыба", согласно задания
